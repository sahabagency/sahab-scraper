...
function actionPlan(service, reason, evidenceClass) {
  const plans = {
    'Content & trust quality': {
      ...
      solution: 'حذف النص غير المرتبط، إعادة كتابة الأسئلة الشائعة حسب المنتجات الفعلية، وربط كل إجابة بصفحة منتج أو فئة.',
      method: 'مراجعة الأسئلة الشائعة والصفحات الأساسية، ثم اختبار الصلة والوضوح على الهاتف وسطح المكتب.',
      measurement: 'انخفاض الصفحات ذات المحتوى غير المرتبط، وتحسن التفاعل مع الأسئلة والنقر إلى الفئات أو المنتجات.'
    },
    'SEO & category demand capture': {
      problem: 'صفحات فئات محددة تفتقد عناصر تحسين الظهور في البحث، مثل العنوان أو الوصف أو العنوان الرئيسي أو الرابط الأساسي.',
      solution: 'إكمال عناصر تحسين الظهور لكل فئة، بناء قالب موحد، وربط الفئات بصفحات المنتجات والبحث الداخلي.',
      method: 'إنشاء جدول للفئات، تجهيز قالب للعناوين والأوصاف، فحص Search Console، ومراجعة الظهور بعد النشر.',
      measurement: 'نسبة اكتمال عناصر الفئات، الصفحات المفهرسة، والنقرات العضوية لكل فئة.'
    },
    'SEO & category demand growth': {
      problem: 'هناك كتالوج وفئات فعلية، لكن بعض عمليات البحث المرتبطة بالشراء قد لا تصل إلى الصفحة أو المنتج المناسب.',
      solution: 'تحسين صفحات الفئات والصفحات المخصصة حسب كلمات البحث والمنتج والاستخدام.',
    ...
    'Search & category demand growth': {
      ...
      solution: 'مراجعة خريطة الكلمات الحالية، تقوية الربط بين المقالات وصفحات الفئات والمنتجات، وإضافة زر واضح من المقالة إلى العرض المناسب.',
      method: 'مطابقة كل مقال وفئة مع موضوع واضح، فحص Search Console والبحث الداخلي، ثم اختبار العنوان والروابط وزر الإجراء على الصفحات ذات الظهور العالي والنقر المنخفض.',
    ...
    'Retention & loyalty activation': {
      ...
      solution: 'تحسين تعريف العميل ببرنامج الولاء عند التسجيل، وتشغيل رسائل تذكير وعروض حسب سلوكه، وربط المكافأة بالشراء المتكرر.',
      method: 'تقسيم الأعضاء حسب آخر شراء، اختبار رسائل التذكير، ومقارنة مجموعات العملاء قبل وبعد التحسين.',
      measurement: 'معدل تفعيل الولاء، الشراء المتكرر، الإيراد لكل عضو، ونسبة استخدام المكافآت.'
    },
    'B2B / project pipeline expansion': {
      problem: 'مسار المشاريع ظاهر، لكن توسيعه يحتاج نموذج طلب عرض سعر ومتابعة يمكن قياسهما.',
      solution: 'إنشاء نموذج مختصر يطلب نوع المشروع والكمية والموعد، مع صفحة مخصصة ورسالة متابعة آلية.',
      method: 'وضع زر طلب عرض سعر واضح في صفحات الشركات، ربط النموذج بالتتبع، تحديد مدة للرد، وتجربة مصادر طلب مختلفة.',
      measurement: 'عدد طلبات عروض الأسعار المؤهلة، زمن الرد، نسبة التحول إلى اجتماع، وقيمة العروض.'
    },
    'Product conversion & trust': {
      problem: 'عينة صفحات المنتجات لا تعرض أدلة الثقة، مثل التقييمات والمراجعات، قرب قرار الشراء بشكل واضح.',
      solution: 'إظهار التقييمات والمراجعات والصور والشحن والضمان قرب قرار الشراء.',
      method: 'اختبار نسختين مختلفتين لترتيب عناصر الثقة، ثم مقارنة النتيجة بالصفحة الحالية.',
      measurement: 'الإضافة إلى السلة، بدء الدفع، ومعدل التحويل لكل صفحة.'
    },
    'Appointment booking funnel': {
      ...
      method: 'وضع زر الحجز في أعلى الصفحة وبطاقات الخدمات وصفحات العروض، ربطه بنموذج أو نظام الحجز، ثم اختبار الحجز من الجوال لكل فرع.',
    ...
    'Service purchase to appointment handoff': {
      problem: 'صفحة الخدمة تسمح بشراء الباقة واختيار الفرع، لكن تعليمات تحديد الموعد بعد الدفع غير ظاهرة بوضوح في الصفحات المفحوصة.',
      solution: 'إظهار ما يحدث بعد الدفع: رسالة تأكيد فورية، رابط واتساب أو اتصال مباشر، مهلة تواصل واضحة، وخيار اختيار الموعد أو طلبه لكل فرع.',
      method: 'إضافة تعليمات بجانب زر الشراء وفي صفحة التأكيد والبريد، تمرير رقم الطلب واسم الخدمة والفرع للفريق، ثم اختبار المسار من الجوال.',
      measurement: 'نسبة الطلبات التي تم التواصل معها، زمن أول رد، نسبة حجز الموعد بعد الشراء، والإلغاءات أو الاسترداد بسبب غموض الخطوة التالية.'
    },
    'Service & offer merchandising / booking CRO': {
      ...
      method: 'تقسيم الفئات إلى مجموعات علاجية، تثبيت أفضل الباقات أعلى الصفحة، إضافة مقارنة مختصرة وزر موحد، ثم اختبار ترتيب البطاقات والرسائل.',
      measurement: 'نسبة النقر من الفئة إلى العرض، بدء الحجز، الضغط على واتساب أو الاتصال، ومعدل التحويل لكل فئة.'
    },
    'Product merchandising & CRO': {
      ...
      solution: 'إظهار المنتجات أو الباقات الأعلى طلبًا، إضافة مقارنة ومراجعات ومعلومات الشحن والضمان قرب السعر، ووضع زر شراء واضح لكل منتج.',
      ...
      measurement: 'نسبة النقر من الفئة للمنتج، الإضافة إلى السلة، بدء الدفع، متوسط قيمة السلة، ومعدل التحويل حسب المنتج.'
    },
    'B2B / project lead capture': {
      problem: 'صفحة المشاريع/الشركات موجودة، لكن طلب عرض سعر مخصص غير ظاهر في العينة.',
      solution: 'إضافة نموذج مختصر لطلب عرض سعر مع نوع المشروع والكمية والموعد ووسيلة التواصل.',
      method: 'وضع زر واضح في صفحة الشركات، ربط النموذج بالتتبع، وإشعار الفريق فور وصول الطلب.',
      measurement: 'عدد طلبات عروض الأسعار المؤهلة، زمن الرد، ونسبة التحول إلى اجتماع أو عرض.'
    }
  };
  const en = {
    ...
    'Search & category demand growth': { problemEn:'The site already has categories and editorial content; the opportunity is to improve their connection to search and purchase intent, not rebuild the keyword plan from zero.', solutionEn:'Review the existing keyword map, strengthen links between articles, categories, and products, and add a CTA from each article to the relevant offer.', methodEn:'Map each article and category to one intent, inspect Search Console and internal search, then test titles, links, and CTAs on high-impression/low-click pages.', measurementEn:'Track organic CTR, qualified keywords, article-to-category clicks, and category-to-product or booking clicks.' },
    ...
    'Service purchase to appointment handoff': { problemEn:'Service pages allow a visitor to buy a package and select a branch, but the post-payment appointment handoff was not clearly observable in sampled pages.', solutionEn:'Show exactly what happens after payment: instant confirmation, WhatsApp/call link, response SLA, and appointment selection/request for each branch.', methodEn:'Place the handoff beside the purchase CTA and in confirmation/email, pass order, service, and branch data to the team, then test the mobile journey.', measurementEn:'Track contacted-after-purchase rate, first-response time, appointment-after-purchase rate, and cancellations/refunds caused by unclear next steps.' },
    ...
    'Product merchandising & CRO': { problemEn:'The catalog and buying path exist; the improvement point is product ordering and reducing hesitation before add-to-cart.', solutionEn:'Surface the highest-demand products or bundles, add comparison/reviews/delivery/warranty details near price, and use a direct CTA on each product.', methodEn:'Use internal search and click data to find high-visit/no-add-to-cart products, then test card order, imagery, offers, and CTA copy on mobile.', measurementEn:'Track category-to-product CTR, add-to-cart, begin_checkout, average order value, and conversion rate by product.' },
...
function verifiedRows(audit={},base){
  ...
  if((clinic||serviceCommerce)&&servicePurchaseDetected&&!f.trackers?.postPurchaseSchedulingDetected){ rows.push(row({service:'Service purchase to appointment handoff',reason:'صفحات الخدمة تُظهر شراء الباقة أو الخدمة، لكن لا توضّح للعميل ماذا يفعل بعد الدفع لتحديد الموعد أو متى سيتواصل معه الفريق. هذه فجوة في وضوح الخطوة التالية بعد الشراء، وليست مشكلة في وجود الحجز الإلكتروني.',lowRate:.002,highRate:.008,confidence:88,evidenceClass:'verified_gap'},base)); }
