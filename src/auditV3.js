...
export async function auditLead(lead,assumptions={}){
  ...
  try{bi.technicalAudit=await buildTechnicalAudit({url:response.url||lead.website,requestedUrl:lead.website||response.url,html,bi});if(bi.technicalAudit.redirect?.domainChanged){bi.confidence=Math.min(Number(bi.confidence||0),45);bi.evidence={...(bi.evidence||{}),domainMismatch:true,requestedUrl:lead.website,finalUrl:response.url};}}catch(error){bi.technicalAudit={state:'unavailable',reason:error.message};}
