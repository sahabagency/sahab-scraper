...
function renderLiveScan(data) {
  const lead = data.lead || {}; const audit = data.audit || {}; const q = data.qualification || audit.qualification || {}; const profile = audit.commercialProfile || {}; const opportunity = audit.opportunity || {}; const monthly = opportunity.monthlyRange || { low: 0, high: 0 }; const annualBreakdown = audit.opportunityBreakdown || []; const route = lead.contactRoute || {}; const confidence = opportunity.confidence ?? route.confidence ?? (lead.contactEmail ? 95 : 70); const showMoney = opportunity.displayEligible === true;
  const rows = annualBreakdown.slice(0, 6).map(item => {
    ...
    return `<div class="leak-row"><div><h3>${esc(item.service)}</h3><p>${esc((item.issues || []).join(' · '))}</p></div><div class="leak-value">${showMoney ? `−${money(high)} ريال<small>${money(low)}–${money(high)} ريال / شهر تقديري</small>` : '<span style="color:#d7bd68">Observed gap</span><small>no money claim without commercial context</small>'}</div></div>`;
  }).join('');
  ...
  const moneyBox = showMoney
    ...
    : `<div class="leak-box"><div class="label">COMMERCIAL ESTIMATE WITHHELD</div><div class="amount" style="font-size:28px;color:#d7bd68">لا يوجد رقم موثوق كفاية</div><div class="range">فحص الرابط وحده لا يكفي لتقدير خسارة مالية بالريال. نحتاج سياقًا تجاريًا مثل نوع النشاط ومتوسط قيمة الطلب وحجم الـleads.</div></div>`;
  const profileBox = showMoney
    ? `<div class="leak-row"><div><h3>Commercial assumption profile</h3><p>متوسط قيمة مفترض ${money(profile.averageTicketRange?.low)}–${money(profile.averageTicketRange?.high)} ريال · leads شهرية modeled ${money(profile.monthlyLeadRange?.low)}–${money(profile.monthlyLeadRange?.high)} · ليست بيانات CRM مؤكدة</p></div><div class="leak-value">${money(profile.confidence || 0)}%<small>assumption confidence</small></div></div>`
    : `<div class="leak-row"><div><h3>Commercial context</h3><p>تم حجب الرقم بدل عرض تقدير مضلل. الفحص التقني يظل صالحًا، لكن القيمة المالية تحتاج مدخلات تجارية إضافية.</p></div><div class="leak-value">WITHHELD<small>credibility gate</small></div></div>`;
  ...
  scanResult.innerHTML = `<div class="scan-result-card">
    ...
    ${moneyBox}${profileBox}
    ${rows || '<div class="leak-row"><div><h3>No reliable customer-facing gap detected</h3><p>Tracking/social non-detection is not counted as a missing feature.</p></div><div class="leak-value">—<small>no false negative claim</small></div></div>'}
    ${unknowns ? `<div class="leak-row"><div><h3>Dynamic integrations — unknown, not missing</h3><ul style="margin:8px 0 0;padding-inline-start:20px">${unknowns}</ul></div><div class="leak-value">NEUTRAL<small>excluded from loss estimate</small></div></div>` : ''}
    ${emailPreview}
  </div>`;
}
...
scanForm.addEventListener('submit', async event => {
  ...
  scanStatus.textContent = 'Scanning website, conversion path, public contact data and observable integrations…'; scanForm.querySelector('button').disabled = true; scanResult.innerHTML = '';
  try {
    ...
    const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Scan failed'); renderLiveScan(data); scanStatus.textContent = 'Live scan complete · public data only · uncertain dynamic integrations are neutral';
...
function renderCampaign(campaign) {
  ...
  results.innerHTML = leads.map((lead,index)=>{
    const audit=lead.audit||{}; const annual=audit.opportunity?.annualRange||{low:0,high:0}; const showMoney = audit.opportunity?.displayEligible === true; const q = lead.qualification || audit.qualification || {}; const profile = audit.commercialProfile || {};
    const issues=(audit.issues||[]).slice(0,4).map(i=>`<li><strong>${esc(i.severity)}</strong> — ${esc(i.title)}</li>`).join('');
    const breakdown=(audit.opportunityBreakdown||[]).slice(0,6).map(item=>`<tr><td>${esc(item.service)}</td><td>${showMoney ? `${money(item.annualRange?.low)}–${money(item.annualRange?.high)} ريال` : 'withheld'}</td><td>${esc((item.issues||[]).slice(0,2).join(', '))}</td></tr>`).join('');
