(() => {
  ...
  function render(data) {
    ...
    const b2bHtml = b2bScenario ? `<p><strong>${isEnglish()?'B2B project path modeled separately:':'مسار المشاريع B2B محسوب منفصلًا:'}</strong> قيمة فرصة نموذجية ${ri(b2bScenario.opportunityValueRange?.low)}–${ri(b2bScenario.opportunityValueRange?.high)} × ${money(b2bScenario.modeledMonthlyOpportunityCount?.low)}–${money(b2bScenario.modeledMonthlyOpportunityCount?.high)} ${isEnglish()?'potential opportunities/month · confidence ':'فرص محتملة بالشهر · ثقة '}${money(b2bScenario.confidence)}% ${isEnglish()?'· not store AOV or confirmed revenue.':'· ليس هذا متوسط طلب المتجر ولا إيرادًا مؤكدًا.'}</p>` : '';
