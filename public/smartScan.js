(() => {
  ...
  function render(data) {
    ...
    const rows = breakdown.slice(0,6).map(item => {
      ...
      return `<div class="leak-row finding"><div><h3>${esc(item.service)}</h3><p><strong>${labels.problem}:</strong> ${esc(english ? (item.problemEn || (englishPlan(item.service).problem) || item.problem || '') : (item.problem || ''))}</p><p><strong>${labels.evidence}:</strong> ${esc(english ? (item.evidenceEn || englishPlan(item.service).problem) : (item.issues || []).join(' · '))}</p><p><strong>${labels.solution}:</strong> ${esc(english ? (item.solutionEn || englishPlan(item.service).solution || '') : (item.solution || ''))}</p><p><strong>${labels.method}:</strong> ${esc(english ? (item.methodEn || englishPlan(item.service).method || '') : (item.method || ''))}</p><p><strong>${labels.measurement}:</strong> ${esc(english ? (item.measurementEn || englishPlan(item.service).measurement || '') : (item.measurement || ''))}</p><p style="font-size:11px;color:#c9b76e">${esc(evidenceTag)} · ${money(item.confidence || 0)}%</p></div><div class="leak-value">${money(hi)}<small>${money(lo)}–${money(hi)} ${esc(labels.month)}</small></div></div>`;
    }).join('');
