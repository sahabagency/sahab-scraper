(() => {
  ...
  const mid = r => r && Number(r.low) > 0 && Number(r.high) > 0 ? Math.round((Number(r.low) + Number(r.high)) / 2) : null;
  ...
  function render(data) {
    ...
    const categories = (bi.categories || []).slice(0,8);
    const products = (bi.products || []).slice(0,8);
    const segments = (bi.targetSegments || []).slice(0,6);
    const values = (bi.valuePropositions || []).slice(0,6);
    const funnel = bi.funnelSignals || {};
    const monthlyTitle = breakdown.length ? 'ESTIMATED MONTHLY REVENUE OPPORTUNITY' : 'MODELED MONTHLY GROWTH OPPORTUNITY';
    ...
    const rows = breakdown.slice(0,6).map(item => {
      ...
      return `<div class="leak-row"><div><h3>${esc(item.service)}</h3><p>${esc((item.issues || []).join(' · '))}</p></div><div class="leak-value">−${money(hi)}<small>${money(lo)}–${money(hi)} ريال / شهر تقديري</small></div></div>`;
    }).join('');
    ...
    result.innerHTML = `<div class="scan-result-card">
      ...
      <div class="leak-row"><div><h3>Commercial model built from this business</h3><p>متوسط قيمة الطلب/الفرصة ${money(profile.averageTicketRange?.low)}–${money(profile.averageTicketRange?.high)} ريال · الطلب الشهري المقدر ${money(profile.monthlyLeadRange?.low)}–${money(profile.monthlyLeadRange?.high)} · مصدر السعر ${esc(profile.averageTicketSource || bi.averageTicketSource || 'inferred')}</p></div><div class="leak-value">${money(profile.confidence || 0)}%<small>ثقة النموذج التجاري</small></div></div>
      ${rows || `<div class="leak-row"><div><h3>لا توجد فجوة تحويل كبيرة مؤكدة حتى الآن</h3><p>الرقم أعلاه نموذج فرصة نمو مبني على نوع المشروع والأسعار ومسار البيع، وليس ادعاء خسارة مؤكدة. النتائج غير القابلة للرصد مثل Pixel داخل GTM تبقى Unknown ولا تُحسب كفجوة.</p></div></div>`}
      ${email}
    </div>`;
  }
  ...
  form.addEventListener('submit', async event => {
    ...
    result.innerHTML = '';
    status.textContent = 'أحلل المشروع من الرابط: النشاط، المنصة، المنتجات، الأسعار، العملاء، مسار البيع ثم أبني التقدير…';
    try {
      // Pass 1: learn the business from the URL with no fake campaign defaults.
      const first = await scan({ lead:{ name:nameFromUrl(website), website }, assumptions:{} });
      const bi = first.audit?.businessIntelligence || {};
      const profile = first.audit?.commercialProfile || {};
      const avgTicket = Number(bi.averageTicketAnchor) || mid(profile.averageTicketRange);
      const monthlyLeads = Number(bi.monthlyLeadAnchor) || mid(profile.monthlyLeadRange);

      // Pass 2: audit again using only business context inferred from that site.
      const second = await scan({
        lead:{ name:bi.brandName || first.lead?.name || nameFromUrl(website), website },
        industry: bi.industry || '',
        assumptions:{
          averageTicket: avgTicket || undefined,
          monthlyLeadEstimate: monthlyLeads || undefined,
          commercialContextProvided: true
        }
      });
      render(second);
      status.textContent = 'Live scan complete · business inferred from URL · public data only · figures are estimates';
    } catch (error) {
