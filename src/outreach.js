...
function businessContext(lead, audit) {
  ...
  const products = (bi.products || []).slice(0, 4).join('، ');
  const parts = [`راجعت ${lead.name} والموقع بشكل فعلي`];
  if (bi.industry) parts.push(`وفهمت النشاط كـ ${bi.industry}`);
  const platform = platformName(bi); if (platform) parts.push(`والمتجر يعمل على ${platform}`);
  if (bi.primaryRevenueMotion) parts.push(`ومسار الإيراد الأساسي هو ${bi.primaryRevenueMotion}`);
