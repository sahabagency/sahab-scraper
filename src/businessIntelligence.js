...
import { analyzeGoogleBusiness } from './googleBusinessInsights.js';
...
function inferIndustry(text=''){
  ...
  const rules=[
    ...
    ['beauty & wellness', /صالون|سبا|تجميل|عناية بالبشرة|beauty|salon|spa|wellness/],
    ['fitness & gym', /نادي رياضي|جيم|لياقة|fitness|gym/],
    ['education & training', /معهد|أكاديمية|دورات|تدريب|مدرسة|academy|training|course/],
    ['real estate', /عقار|عقارات|فلل|شقق|real estate|property/],
    ['professional services', /استشارات|محاماة|محاسبة|وكالة|تسويق|consulting|law firm|accounting|agency/],
    ['home & field services', /صيانة|تنظيف|نقل|مقاولات|سباكة|maintenance|cleaning|contractor|plumbing/],
    ['hotel & travel', /فندق|شقق فندقية|سفر|حجوزات|hotel|travel|booking/],
    ['automotive', /سيارات|ورشة|إطارات|automotive|garage|tires/],
    ['technology & SaaS', /برنامج|منصة|تقنية|software|saas|platform/],
...
async function lookupGoogleBusiness(brandName,url,lead={}){
  if(lead.googleRating!=null || lead.rating!=null || !process.env.GOOGLE_MAPS_API_KEY) return lead;
  try{
    const host=new URL(url).hostname.replace(/^www\./,'');
    const q=encodeURIComponent(`${brandName||host} ${host}`);
    const r=await fetch(`https://maps.googleapis.com/maps/api/place/textsearch/json?query=${q}&key=${process.env.GOOGLE_MAPS_API_KEY}`,{signal:AbortSignal.timeout(7000)});
    const d=await r.json(); const p=d.results?.[0]; if(!p?.place_id) return lead;
    const fields=encodeURIComponent('name,website,formatted_phone_number,url,business_status,opening_hours,types,reviews,user_ratings_total,rating');
    const dr=await fetch(`https://maps.googleapis.com/maps/api/place/details/json?place_id=${encodeURIComponent(p.place_id)}&fields=${fields}&key=${process.env.GOOGLE_MAPS_API_KEY}`,{signal:AbortSignal.timeout(7000)});
    const detail=(await dr.json()).result||p;
    return {...lead,googleRating:detail.rating??p.rating??null,googleReviewCount:detail.user_ratings_total??p.user_ratings_total??0,googleMapsUrl:detail.url||null,businessStatus:detail.business_status||null,googleTypes:detail.types||p.types||[],googleOpeningHours:detail.opening_hours||null,googleReviews:(detail.reviews||[]).slice(0,5).map(x=>({rating:x.rating,text:x.text,relativeTimeDescription:x.relative_time_description,time:x.time,authorName:x.author_name}))};
  }catch{return lead;}
}
...
function pageSignals(html='',url=''){const $=cheerio.load(html),body=norm($('body').text()),hrefs=$('a[href]').map((_,a)=>$(a).attr('href')||'').get().join(' '),text=`${body} ${hrefs} ${html}`.toLowerCase(),interactive=$('a[href],button,[role="button"],input[type="submit"],input[type="button"],form').map((_,el)=>`${$(el).text()} ${$(el).attr('aria-label')||''} ${$(el).attr('title')||''} ${$(el).attr('href')||''} ${$(el).attr('action')||''}`).get().join(' '),service=/جلسة|جلسات|باقة|علاج|ليزر|بوتكس|فيلر|أسنان|تنظيف|تبييض|treatment|service|package|clinic/i.test(text),branchSelection=/اختيار الفرع|اختر الفرع|الفروع|فرع|branch/i.test(text),paymentMethods=/تمارا|تابي|tamara|tabby|apple pay|mada|stc pay|الدفع الإلكتروني|وسائل الدفع/i.test(text),postPurchaseScheduling=/بعد الشراء|بعد الدفع|سيتواصل معك|سيتواصل|يتواصل معك|تحديد الموعد|تنسيق الموعد|موعدك|schedule.*after|we will contact/i.test(text),discountDetected=/خصم|بدل|discount|sale|original-price|line-through|السعر قبل/i.test(text),urgencyDetected=/ينتهي|متبقي|countdown|timer|عرض اليوم|limited time/i.test(text),loyaltyOfferDetected=/كاش.?باك|cashback|نقاط|points|اكسب/i.test(text);return{url,cart:/أضف إلى السلة|أضف للسلة|add to cart|buy now|سلة المشتريات/.test(text),checkout:/checkout|إتمام الطلب|اتمام الطلب|الدفع|cart/.test(text),quote:/اطلب عرض سعر|طلب عرض سعر|عرض سعر|request a quote|rfq|get quote/.test(text),contact:/whatsapp|واتساب|wa\.me|tel:|تواصل|contact/.test(text),booking:/احجز|حجز موعد|موعد|booking|appointment|book now|schedule/i.test(interactive),form:$('form').length>0,reviews:/تقييمات المنتج|آراء العملاء|اراء العملاء|reviews|rating/.test(text),service,branchSelection,paymentMethods,postPurchaseScheduling,discountDetected,urgencyDetected,loyaltyOfferDetected,title:norm($('h1').first().text()||$('title').text()),soldCounts:[...body.matchAll(/تم\s+شراؤه\s+([0-9٠-٩۰-۹]+)\s*مر/gi)].map(m=>Number(latinDigits(m[1]))).filter(Number.isFinite)};}
...
export async function inferBusinessIntelligence({html='',url='',lead={},assumptions={}}={}){
  ...
  const brandName=extractBrandName($,url),platform=detectPlatform(html); lead=await lookupGoogleBusiness(brandName,url,lead); let categories=extractCategories($),combinedText=`${title} ${meta} ${body} ${categories.join(' ')}`;let industry=inferIndustry(combinedText);
  ...
  industry=inferIndustry(combinedText);const home=pageSignals(html,url),b2bMention=/حلول المشاريع|الشركات|توريد|مقاول|مقاولين|مشاريع/.test(combinedText.toLowerCase()),b2bSignals=sampledSignals.filter(x=>x.type==='b2b'),productSignals=sampledSignals.filter(x=>x.type==='product');
  const b2bPageDetected=links.b2b.length>0,b2bConversionDetected=b2bSignals.some(x=>x.quote||x.contact||x.form),quoteRequestDetected=b2bSignals.some(x=>x.quote)||home.quote,cartDetected=home.cart||productSignals.some(x=>x.cart),checkoutDetected=home.checkout||productSignals.some(x=>x.checkout),reviewsDetected=home.reviews||productSignals.some(x=>x.reviews),bookingDetected=home.booking||sampledSignals.some(x=>x.booking),bookingHomeDetected=home.booking,soldCounts=sampledSignals.flatMap(x=>x.soldCounts||[]),serviceSignals=productSignals.filter(x=>x.service),servicePurchaseDetected=serviceSignals.some(x=>x.cart||x.checkout),branchSelectionDetected=serviceSignals.some(x=>x.branchSelection),paymentMethodsDetected=serviceSignals.some(x=>x.paymentMethods),postPurchaseSchedulingDetected=serviceSignals.some(x=>x.postPurchaseScheduling),discountDetected=serviceSignals.some(x=>x.discountDetected),urgencyDetected=serviceSignals.some(x=>x.urgencyDetected),loyaltyOfferDetected=serviceSignals.some(x=>x.loyaltyOfferDetected),appointmentModel=bookingDetected?'online_booking':servicePurchaseDetected?(postPurchaseSchedulingDetected?'purchase_then_scheduled':'purchase_then_contact_not_observed'):(home.contact?'contact_first':'not_observed');
  const trackers=detectTrackers(html);
  trackers.servicePurchaseDetected=servicePurchaseDetected; trackers.branchSelectionDetected=branchSelectionDetected; trackers.paymentMethodsDetected=paymentMethodsDetected; trackers.postPurchaseSchedulingDetected=postPurchaseSchedulingDetected; trackers.discountDetected=discountDetected; trackers.urgencyDetected=urgencyDetected; trackers.loyaltyOfferDetected=loyaltyOfferDetected; trackers.appointmentModel=appointmentModel;
  // Platform detection is descriptive only. Shopify/Salla can also power an informational
  // or lead-generation site, so mode requires observable purchase evidence.
  const catalogEvidence=productNames.length>=2 || links.products.length>=2 || priceSamples.length>=2;
  const directCommerce=Boolean(cartDetected||checkoutDetected||catalogEvidence);
  const serviceCommerce=Boolean(servicePurchaseDetected || (home.service && (home.booking || home.paymentMethods) && (priceSamples.length>=1 || productNames.length>=1)));
  const leadPath=Boolean(bookingDetected||quoteRequestDetected||home.contact||b2bPageDetected||b2bMention);
  const siteMode=serviceCommerce?'service_commerce':directCommerce?'ecommerce_store':(b2bMention&&!leadPath?'b2b_project_site':leadPath?'lead_generation_service':'informational_website');
  const revenueMotion=directCommerce?(serviceCommerce?'service purchase online':'product purchase online'):b2bMention?'B2B lead generation':(bookingDetected||home.contact)?'lead generation / booking':'information / brand presence';
  const primaryRevenueMotion=revenueMotion,secondaryRevenueMotions=[];if(directCommerce&&b2bMention)secondaryRevenueMotions.push('projects / B2B supply');
  const googleBusinessProfile=analyzeGoogleBusiness({rating:lead.googleRating ?? lead.rating,reviewCount:lead.googleReviewCount ?? lead.reviewCount,reviews:lead.googleReviews,mapsUrl:lead.googleMapsUrl,status:lead.businessStatus,types:lead.googleTypes});
  const businessModel=siteMode==='service_commerce'?(b2bMention?'service commerce + B2B/project sales':'service commerce'):siteMode==='ecommerce_store'?(b2bMention?'ecommerce + B2B/project sales':'ecommerce'):siteMode==='b2b_project_site'?'projects/B2B lead generation':siteMode==='lead_generation_service'?'lead generation / booking':'informational website';
  ...
  return{brandName,industry,siteMode,businessType:siteMode,businessModel,primaryRevenueMotion,secondaryRevenueMotions,platform,currency:currency||'unknown',categories,productFamilies:canonicalFamilies,products:productNames,targetSegments,valuePropositions,googleBusinessProfile,routing:{siteMode,platform:platform||'unknown',industry,enabledModules:siteMode==='ecommerce_store'?['technical_seo','on_page_seo','catalog','product_cro','checkout','tracking']:siteMode==='service_commerce'?['technical_seo','on_page_seo','service_offers','booking_handoff','local_seo','tracking']:siteMode==='lead_generation_service'?['technical_seo','on_page_seo','service_pages','lead_forms','local_seo','tracking']:siteMode==='b2b_project_site'?['technical_seo','on_page_seo','b2b_pages','rfq','lead_forms','tracking']:['technical_seo','on_page_seo','content','contact_path'],evidence:{purchaseButtons:cartDetected,checkoutSignals:checkoutDetected,catalogEvidence,servicePurchase:servicePurchaseDetected,booking:bookingDetected,b2b:b2bMention}},funnelSignals:{cartDetected,checkoutDetected,b2bDetected:!directCommerce&&b2bMention,b2bSecondaryDetected:directCommerce&&b2bMention,b2bPageDetected,b2bConversionDetected,quoteRequestDetected,whatsappDetected:/واتساب|whatsapp|wa\.me/i.test(combinedText),phoneDetected:/tel:|\+966|05[0-9]{8}/i.test(combinedText),reviewsDetected,blogDetected:links.blog.length>0||/\/blog|المدونة|مقالات/i.test(combinedText),loyaltyDetected:/برامج الولاء|نقاط الولاء/i.test(combinedText),installmentDetected:/تمارا|تابي|tamara|tabby|mispay/i.test(combinedText),freeDeliveryDetected:/توصيل سريع ومجاني|شحن مجاني|توصيل مجاني/i.test(combinedText),bookingDetected,bookingHomeDetected,trackers},pageInventory:{productLinks:links.products.length,categoryLinks:links.categories.length,b2bLinks:links.b2b.length,blogLinks:links.blog.length},publicSalesSignals:{purchaseCounters:soldCounts,note:soldCounts.length?'Public purchase counters were observed and used only as a bounded activity signal, never as monthly orders.':'No reliable public order volume was found.'},contentQuality,priceSamples:cleanPrices.slice(0,30),priceStats,coreProductPriceStats:corePriceStats,averageTicketAnchor:inferredTicket,averageTicketSource:ticketSource,monthlyLeadAnchor:inferredMonthlyLeads,monthlyLeadSource:leadSource,confidence,sampledPages,evidence:{title,metaDescription:meta,categoryCount:categories.length,productCount:productNames.length,productFamilyCount:canonicalFamilies.length,priceSampleCount:cleanPrices.length,corePriceSampleCount:corePriceStats?.sampleCount||0,sampledPageCount:sampledPages.length,brandName,primaryRevenueMotion,siteMode},disclaimer:'Business type, product mix, public pricing, platform, funnel and target segments are inferred from publicly accessible pages. Google rating/reviews are included only when connected data provides them. Public purchase counters are activity signals only; traffic, orders, ad spend, conversion rate and realized uplift remain unknown until first-party data is connected.'};
}
