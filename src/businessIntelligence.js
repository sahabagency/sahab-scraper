...
function extractBrandName($, url=''){
  const candidates=[$('meta[property="og:site_name"]').attr('content'),$('meta[name="application-name"]').attr('content'),$('header img[alt]').first().attr('alt'),$('title').text().split(/[|–—-]/)[0]].map(norm).filter(Boolean);
  const chosen=candidates.find(x=>x.length>=3&&x.length<=90&&!/^(home|الرئيسية|متجر|store)$/i.test(x));
  if(chosen)return chosen; try{return new URL(url).hostname.replace(/^www\./,'').split('.')[0].replace(/[-_]+/g,' ');}catch{return null;}
}

function extractJsonLdProducts($){
  const prices=[]; const names=[]; const currencies=[];
  $('script[type="application/ld+json"]').each((_,el)=>{ try{ const parsed=JSON.parse($(el).html()||'{}'); const walk=node=>{ if(!node||typeof node!=='object')return; const type=Array.isArray(node['@type'])?node['@type'].join(' '):String(node['@type']||''); if(/product/i.test(type)){ if(node.name)names.push(norm(node.name)); const offers=Array.isArray(node.offers)?node.offers:[node.offers].filter(Boolean); for(const offer of offers){ const n=Number(latinDigits(String(offer?.price||offer?.lowPrice||'')).replace(/,/g,'')); if(Number.isFinite(n)&&n>=10&&n<=500000)prices.push(n); if(offer?.priceCurrency)currencies.push(String(offer.priceCurrency).toUpperCase()); }} for(const value of Object.values(node)){ if(Array.isArray(value))value.forEach(walk); else if(value&&typeof value==='object')walk(value); }}; (Array.isArray(parsed)?parsed:[parsed]).forEach(walk); }catch{} });
...
async function fetchText(url){try{const r=await fetch(url,{redirect:'follow',headers:{'user-agent':'Mozilla/5.0 (compatible; SahabBusinessIntelligence/3.1; +https://sahab.agency)'},signal:AbortSignal.timeout(10000)});if(!r.ok)return null;return{html:await r.text(),finalUrl:r.url};}catch{return null;}}
function pageSignals(html='',url=''){const $=cheerio.load(html),body=norm($('body').text()),hrefs=$('a[href]').map((_,a)=>$(a).attr('href')||'').get().join(' '),text=`${body} ${hrefs} ${html}`.toLowerCase();return{url,cart:/أضف إلى السلة|أضف للسلة|add to cart|buy now|سلة المشتريات/.test(text),checkout:/checkout|إتمام الطلب|اتمام الطلب|الدفع|cart/.test(text),quote:/اطلب عرض سعر|طلب عرض سعر|عرض سعر|request a quote|rfq|get quote/.test(text),contact:/whatsapp|واتساب|wa\.me|tel:|تواصل|contact/.test(text),form:$('form').length>0,reviews:/تقييمات المنتج|آراء العملاء|اراء العملاء|reviews|rating/.test(text),title:norm($('h1').first().text()||$('title').text()),soldCounts:[...body.matchAll(/تم\s+شراؤه\s+([0-9٠-٩۰-۹]+)\s*مر/gi)].map(m=>Number(latinDigits(m[1]))).filter(Number.isFinite)};}
...
export async function inferBusinessIntelligence({html='',url='',lead={},assumptions={}}={}){
  ...
  const jsonLd=extractJsonLdProducts($);let priceSamples=uniq([...jsonLd.prices,...extractPagePrices($,html)]).filter(n=>n>=10&&n<=500000),productNames=uniq(jsonLd.names),currencies=[...jsonLd.currencies];
  const links=classifyInternalLinks($,url),sampledPages=[],sampledSignals=[],sampleUrls=uniq([...links.products.slice(0,6),...links.categories.slice(0,2),...links.b2b.slice(0,2)]).slice(0,10),fetched=await Promise.all(sampleUrls.map(fetchText));
  fetched.forEach((page,i)=>{if(!page)return;const candidate=sampleUrls[i];sampledPages.push(page.finalUrl||candidate);const p$=cheerio.load(page.html),pBody=norm(p$('body').text()),pJson=extractJsonLdProducts(p$);priceSamples=uniq([...priceSamples,...pJson.prices,...extractPagePrices(p$,page.html)]).filter(n=>n>=10&&n<=500000).slice(0,160);productNames=uniq([...productNames,...pJson.names,/\/p\d+\/?$/i.test(new URL(candidate).pathname)?norm(p$('h1').first().text()):null]).slice(0,40);currencies=uniq([...currencies,...pJson.currencies]);categories=uniq([...categories,...extractCategories(p$)]).slice(0,30);combinedText+=` ${pBody.slice(0,10000)}`;sampledSignals.push({...pageSignals(page.html,page.finalUrl||candidate),type:links.b2b.includes(candidate)?'b2b':links.products.includes(candidate)?'product':'category'});});
  ...
  const businessModel=directCommerce?'ecommerce':b2bMention?'projects/B2B lead generation':'lead_generation_or_info';
  const currency=extractCurrency(`${combinedText} ${html}`,currencies)||(url&&/\.sa(?:\/|$)/i.test(url)?'SAR':null),targetSegments=inferSegments(combinedText),valuePropositions=extractValueProps(combinedText),cleanPrices=uniq(priceSamples).filter(n=>n>=10&&n<=500000),priceStats=cleanPrices.length?{min:Math.min(...cleanPrices),p25:percentile(cleanPrices,.25),median:median(cleanPrices),p75:percentile(cleanPrices,.75),max:Math.max(...cleanPrices),sampleCount:cleanPrices.length,samples:cleanPrices.slice(0,40)}:null;
  let inferredTicket=Number(assumptions.averageTicket)||null,ticketSource=assumptions.averageTicket?'campaign_anchor':null;const observedTicket=ticketFromPrices(cleanPrices,industry);if(observedTicket){inferredTicket=observedTicket;ticketSource='observed_public_product_prices';}else if(!inferredTicket&&directCommerce){inferredTicket=/water coolers|tanks/i.test(industry)?2500:/perfume/i.test(industry)?350:450;ticketSource='industry_prior';}
  let inferredMonthlyLeads=Number(assumptions.monthlyLeadEstimate)||null,leadSource=assumptions.monthlyLeadEstimate?'campaign_anchor':null;if(!inferredMonthlyLeads){const reviews=Math.max(0,Number(lead.reviewCount)||0);if(reviews){inferredMonthlyLeads=clamp(Math.round(Math.sqrt(reviews)*2),8,100);leadSource='bounded_review_activity_proxy';}else if(soldCounts.length){inferredMonthlyLeads=clamp(Math.max(...soldCounts)*2,8,35);leadSource='public_purchase_counter_activity_envelope';}else{inferredMonthlyLeads=directCommerce?18:12;leadSource='business_model_prior';}}
  const contentQuality=[];if(/water coolers|tanks/i.test(industry)&&/صيحات الموضة|fashion trends|مجلات الموضة/i.test(combinedText))contentQuality.push({type:'irrelevant_faq',severity:'medium',evidence:'Fashion-trend FAQ text appears on a water-cooler/tank storefront.'});
  let confidence=32;if(industry!=='general business')confidence+=12;if(platform)confidence+=10;if(productNames.length>=2)confidence+=10;if(cleanPrices.length>=3)confidence+=16;else if(cleanPrices.length)confidence+=8;if(categories.length>=2)confidence+=6;if(sampledPages.length>=3)confidence+=7;if(currency)confidence+=3;if(primaryRevenueMotion)confidence+=4;confidence=clamp(confidence,30,95);
  return{brandName,industry,businessModel,primaryRevenueMotion,secondaryRevenueMotions,platform,currency:currency||'unknown',categories,products:productNames,targetSegments,valuePropositions,funnelSignals:{cartDetected,checkoutDetected,b2bDetected:!directCommerce&&b2bMention,b2bSecondaryDetected:directCommerce&&b2bMention,b2bPageDetected,b2bConversionDetected,quoteRequestDetected,whatsappDetected:/واتساب|whatsapp|wa\.me/i.test(combinedText),phoneDetected:/tel:|\+966|05[0-9]{8}/i.test(combinedText),reviewsDetected,blogDetected:links.blog.length>0||/\/blog|المدونة|مقالات/i.test(combinedText),loyaltyDetected:/برامج الولاء|نقاط الولاء/i.test(combinedText),installmentDetected:/تمارا|تابي|tamara|tabby|mispay/i.test(combinedText),freeDeliveryDetected:/توصيل سريع ومجاني|شحن مجاني|توصيل مجاني/i.test(combinedText)},pageInventory:{productLinks:links.products.length,categoryLinks:links.categories.length,b2bLinks:links.b2b.length,blogLinks:links.blog.length},publicSalesSignals:{purchaseCounters:soldCounts,note:soldCounts.length?'Public purchase counters were observed, but are not treated as monthly orders.':'No reliable public order volume was found.'},contentQuality,priceSamples:cleanPrices.slice(0,30),priceStats,averageTicketAnchor:inferredTicket,averageTicketSource:ticketSource,monthlyLeadAnchor:inferredMonthlyLeads,monthlyLeadSource:leadSource,confidence,sampledPages,evidence:{title,metaDescription:meta,categoryCount:categories.length,productCount:productNames.length,priceSampleCount:cleanPrices.length,sampledPageCount:sampledPages.length,brandName,primaryRevenueMotion},disclaimer:'Business type, product mix, public pricing, platform, funnel and target segments are inferred from publicly accessible pages. Public purchase counters are activity signals only; demand and conversion impact remain modeled until first-party analytics/order data is available.'};
}
