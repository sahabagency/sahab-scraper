(() => {
  ...
  function sourceLabel(value='') {
    const map = {
      observed_site_prices:'أسعار المنتجات المرصودة من المتجر', observed_public_product_prices:'أسعار المنتجات المرصودة من المتجر',
      ...
      business_model_prior:'تقدير محافظ حسب نموذج النشاط', category_demand_envelope:'تقدير محافظ حسب نوع النشاط', public_activity_demand_envelope:'نشاط عام + بصمة المتجر',
      campaign_anchor:'قيمة مدخلة في الحملة', campaign_anchor_plus_public_activity:'قيمة حملة + نشاط عام'
    };
  ...
  function render(data) {
    ...
    const baselineHtml = baseline.monthlyCommerceScenarioRange ? `<p><strong>سيناريو القيمة التجارية الشهرية قبل التحسين:</strong> ${ri(baseline.monthlyCommerceScenarioRange.low)}–${ri(baseline.monthlyCommerceScenarioRange.high)} · مبني على وسيط سعر مرصود ${ri(baseline.medianObservedTicket)} ونطاق طلب/طلبات ${money(baseline.modeledOrderRange?.low)}–${money(baseline.modeledOrderRange?.high)} شهريًا.</p>` : '';
    ...
    result.innerHTML = `<div class="scan-result-card">
      ...
      <div class="leak-row" style="display:block"><h3>Commercial model built from this business</h3><p>متوسط قيمة الطلب/الفرصة ${ri(profile.averageTicketRange?.low)}–${ri(profile.averageTicketRange?.high)} · الطلب/الفرص الشهرية المقدر ${money(profile.monthlyLeadRange?.low)}–${money(profile.monthlyLeadRange?.high)} · مصدر القيمة: ${esc(sourceLabel(profile.averageTicketSource || bi.averageTicketSource))} · مصدر الطلب: ${esc(sourceLabel(profile.monthlyLeadSource || bi.monthlyLeadSource))}</p>${baselineHtml}${upliftHtml}<p style="color:#9d9789">الرقم النهائي لا يحسب Pixel أو السوشيال كفجوات إذا كانت غير قابلة للرصد، ولا يجعل B2B هو النشاط كله إذا المتجر يبيع مباشرة للمستهلك.</p></div>
