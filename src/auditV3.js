...
export async function auditLead(lead,assumptions={}){
  ...
  try{bi.technicalAudit=await buildTechnicalAudit({url:response.url||lead.website,requestedUrl:lead.requestedWebsite||lead.website||response.url,html,bi});if(bi.technicalAudit.redirect?.domainChanged){bi.confidence=Math.min(Number(bi.confidence||0),45);bi.evidence={...(bi.evidence||{}),domainMismatch:true,requestedUrl:lead.requestedWebsite||lead.website,finalUrl:response.url};if(bi.googleBusinessProfile)bi.googleBusinessProfile={...bi.googleBusinessProfile,matchStatus:'needs_verification',limitations:'Google Business data was found, but the submitted domain redirected to another host; verify the profile belongs to the submitted business before using its rating or reviews.'};}}catch(error){bi.technicalAudit={state:'unavailable',reason:error.message};}
