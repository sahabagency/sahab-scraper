...
app.get('/health', (_req, res) => res.json({ ok: true, service: 'Sahab X-Ray Lead Engine', persistence: dbConfigured() ? 'supabase' : 'memory', now: new Date().toISOString() }));

app.get('/api/config', async (_req, res) => {
  const gmail = gmailStatus(); let persistenceStats = null;
...
app.get('/api/gmail/verify', async (_req, res) => { try { const profile = await verifyGmailConnection(); res.json({ ok: true, connected: true, emailAddress: profile.emailAddress }); } catch (error) { res.status(400).json({ ok: false, connected: false, error: error.message || 'Gmail verification failed' }); } });
app.get('/api/calendar/verify', async (_req, res) => { try { await verifyCalendarConnection(); res.json({ ok: true, connected: true }); } catch (error) { res.status(400).json({ ok: false, connected: false, error: error.message || 'Calendar verification failed' }); } });
app.get('/auth/google', (_req, res) => { try { res.redirect(createGoogleAuthUrl()); } catch (error) { res.status(500).send(`Google OAuth setup error: ${String(error.message || error)}`); } });
...
app.get('/auth/google/callback', async (req, res) => {
  ...
  } catch (error) { res.status(400).type('html').send(`<h2>Google OAuth failed</h2><pre>${String(error.message || error)}</pre>`); }
});
...
async function handleUnsubscribe(req, res, oneClick = false) {
  ...
  } catch { res.status(400).type('text/plain').send('Invalid or expired unsubscribe link'); }
}
...
app.post('/api/leads/:id/send', async (req, res) => {
  const policy = outboundPolicy(); if (!dbConfigured()) return res.status(503).json({ ok: false, error: 'persistent database required before sending' }); if (!policy.enabled) return res.status(403).json({ ok: false, error: 'outbound_disabled' }); if (policy.requireReview && req.body?.approved !== true) return res.status(409).json({ ok: false, error: 'review_required' });
  let claim = null;
  try {
...
app.post('/api/leads/suppress', async (req, res) => { try { if (!dbConfigured()) return res.status(503).json({ ok: false, error: 'persistent database required' }); if (!req.body?.email) return res.status(400).json({ ok: false, error: 'email required' }); await suppressEmail(req.body.email, req.body.reason || 'manual'); res.json({ ok: true }); } catch (error) { res.status(500).json({ ok: false, error: error.message }); } });
app.post('/api/automation/check-replies', async (_req, res) => { try { res.json(await runReplyAndFollowupCheck()); } catch (error) { res.status(500).json({ ok: false, error: error.message }); } });
app.get('/api/system/status', async (_req, res) => { try { res.json({ ok: true, persistence: dbConfigured() ? await dbStatus() : null, automation: automationStatus(), outbound: { ...outboundRuntimeStatus(), persistence: dbConfigured() ? 'supabase' : 'memory_only' } }); } catch (error) { res.status(500).json({ ok: false, error: error.message }); } });
...
async function runStartupSelfTest() {
  ...
  try {
    console.log('[SELFTEST] starting Riyadh aesthetic clinics dry-run (no email send)'); const location = 'Riyadh, Saudi Arabia'; const industry = 'aesthetic clinics'; const leads = await discoverLeads({ industry, location, limit: 3 }); const results = [];
    for (const rawLead of leads) { const lead = await enrichLeadContact(rawLead, { location }); const audit = await auditLead(lead, { averageTicket: 2500, monthlyLeadEstimate: 40, industry }); const qualification = qualifyLead({ lead, audit }); audit.qualification = qualification; const outreach = await buildOutreach({ lead, audit, bookingUrl: process.env.CALENDAR_BOOKING_URL || '', industry, location }); results.push({ name: lead.name, website: lead.website || null, contactEmail: lead.contactEmail || null, score: audit.score, qualification: { tier: qualification.tier, score: qualification.score }, opportunity: audit.opportunity?.annualRange || null, generatedBy: outreach.generatedBy || null, subject: outreach.subject, bookingLinkIncluded: Boolean(process.env.CALENDAR_BOOKING_URL && outreach.body?.includes(process.env.CALENDAR_BOOKING_URL)) }); }
...
app.listen(PORT, () => {
  ...
  startAutomationLoop(); runStartupSelfTest();
});
