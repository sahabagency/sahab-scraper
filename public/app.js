...
function businessIntelBox(bi = {}) {
  ...
  const prices = (bi.priceSamples || []).slice(0, 6).map(x => `${money(x)} ريال`).join(' · ');
  return `<div class="leak-row"><div><h3>Business intelligence</h3><p><strong>${esc(bi.industry || 'غير محسوم')}</strong> · ${esc(bi.businessModel || '')}${bi.platform ? ` · منصة ${esc(bi.platform)}` : ''}</p>${cats ? `<p>المنتجات/الأقسام المرصودة: ${esc(cats)}</p>` : ''}${prices ? `<p>عينات أسعار عامة: ${esc(prices)}</p>` : ''}<p>مصدر متوسط القيمة: ${esc(bi.averageTicketSource || 'غير محدد')} · مصدر حجم الطلب: ${esc(bi.monthlyLeadSource || 'غير محدد')}</p></div><div class="leak-value">${money(bi.confidence || 0)}%<small>business-context confidence</small></div></div>`;
}
...
function renderLiveScan(data) {
  ...
  const rows = annualBreakdown.slice(0, 6).map(item => {
    ...
    return `<div class="leak-row"><div><h3>${esc(item.service)}</h3><p>${esc((item.issues || []).join(' · '))}</p></div><div class="leak-value">${showMoney ? `−${money(high)} ريال<small>${money(low)}–${money(high)} ريال / شهر تقديري</small>` : '<span style="color:#d7bd68">Observed gap</span><small>commercial confidence below display gate</small>'}</div></div>`;
  }).join('');
  ...
  const moneyBox = showMoney
    ...
    : `<div class="leak-box"><div class="label">ESTIMATED COMMERCIAL OPPORTUNITY</div><div class="amount" style="font-size:28px;color:#d7bd68">تحليل تجاري قيد الثقة</div><div class="range">النظام استنتج نوع المشروع ومنتجاته ومنصته، لكن ثقة التسعير/الطلب الحالية أقل من حد عرض رقم مالي.</div></div>`;
  const profileBox = `<div class="leak-row"><div><h3>Commercial model</h3><p>متوسط قيمة modeled ${money(profile.averageTicketRange?.low)}–${money(profile.averageTicketRange?.high)} ريال · طلب/lead شهري modeled ${money(profile.monthlyLeadRange?.low)}–${money(profile.monthlyLeadRange?.high)} · ليست بيانات داخلية</p></div><div class="leak-value">${money(profile.confidence || 0)}%<small>commercial-model confidence</small></div></div>`;
  ...
  scanResult.innerHTML = `<div class="scan-result-card">
    ...
    ${unknowns ? `<div class="leak-row"><div><h3>Dynamic integrations — unknown, not missing</h3><ul style="margin:8px 0 0;padding-inline-start:20px">${unknowns}</ul></div><div class="leak-value">NEUTRAL<small>excluded from opportunity model</small></div></div>` : ''}
    ${emailPreview}
  </div>`;
}
...
scanForm.addEventListener('submit', async event => {
  ...
  scanStatus.textContent = 'Understanding the business, products, platform, public prices, conversion path, contacts and observable integrations…'; scanForm.querySelector('button').disabled = true; scanResult.innerHTML = '';
  try {
    ...
    const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Scan failed'); renderLiveScan(data); scanStatus.textContent = 'Live intelligence scan complete · public data only · figures are modeled estimates';
